...
export function DietAssignmentDialog({
  ...
  const { clients, assignDiet, diets } = useAppStore()
  ...
  const handleAssign = () => {
    if (!dietId || selectedStudents.length === 0) {
    ...
    toast.success(`Dieta atribuída para ${selectedStudents.length} aluno(s)!`)
    onOpenChange(false)
    setSelectedStudents([])
    setIsLifetime(false)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Atribuir Dieta: {diet?.title}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="flex items-center gap-2 py-2">
            ...
            />
            <Label htmlFor="lifetime-diet">Acesso Vitalício</Label>
          </div>
